...
import { uploadLocalFileToCloudinary } from '../services/cloudinary.service.js'
import fs from 'fs'
...
// Ownership is now a single field, so "my courses" = courses I currently own.
...
export const getStudentDashboard = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // Only courses that are currently claimed by an instructor can be enrolled in.
...
// ─── POST /courses/create ─────────────────────────────────────────────────
// Blocks creating a course with a title that already exists anywhere in
// the DB (case-insensitive, trimmed) — including seeded courses, since
...
export const createCourse = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    if (thumbFile) {
      try {
        img = await uploadLocalFileToCloudinary(thumbFile.path, 'image', 'thumbnails')
        cleanupFile(thumbFile.path)
      } catch {
        cleanupFile(thumbFile.path)
      }
    }
    ...
    if (videoFile) {
      try {
        finalVideoUrl = await uploadLocalFileToCloudinary(videoFile.path, 'video', 'videos')
    ...
    const course = await CourseModel.create({
      ...
      topics: [],
      instructor: instructorId, // creator automatically owns/claims it
      enrolledStudents: [],
    })
...
export const enrollInCourse = async (
  ...
  try {
    ...
    // A course with no owning instructor isn't available for enrollment yet.
...
export const uploadCourseMaterial = async (
  ...
  try {
    const slugOrId = getSlugOrId(req.params.slugOrId)
    const { topicTitle, emoji, subtopicTitle, quiz, notes } = req.body
    ...
    if (files?.videoFile?.[0]) {
      try {
        videoUrl = await uploadLocalFileToCloudinary(files.videoFile[0].path, 'video', 'videos')
        cleanupFile(files.videoFile[0].path)
      } catch (e) {
        cleanupFile(files.videoFile[0].path)
        throw new ApiError('Video upload failed', 500)
      }
    ...
    if (files?.pdfFile?.[0]) {
      try {
        pdfUrl = await uploadLocalFileToCloudinary(files.pdfFile[0].path, 'raw', 'pdfs')
        cleanupFile(files.pdfFile[0].path)
      } catch (e) {
        cleanupFile(files.pdfFile[0].path)
        throw new ApiError('PDF upload failed', 500)
      }
    }
    ...
    if (targetTopic) {
      updatedCourse = await CourseModel.findOneAndUpdate(
        { _id: course._id, 'topics._id': targetTopic._id },
...
export const updateCourse = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    // If the title is changing, re-check for duplicates (excluding this course itself).
    ...
    if (thumbFile) {
      try {
        updateData.img = await uploadLocalFileToCloudinary(thumbFile.path, 'image', 'thumbnails')
        cleanupFile(thumbFile.path)
      } catch {
        cleanupFile(thumbFile.path)
      }
    ...
    if (videoFile) {
      try {
        updateData.videoUrl = await uploadLocalFileToCloudinary(videoFile.path, 'video', 'videos')
        cleanupFile(videoFile.path)
      } catch {
        cleanupFile(videoFile.path)
        throw new ApiError('Video upload failed', 500)
...
// Removes the course from the database entirely. Only the current owner can do this.
...
// Claims an unowned (instructor === null) course. If it's already owned
// by someone else, this fails — only one instructor can hold a course
// at a time.
...
// Removes the course from the current owner's dashboard WITHOUT deleting
// it — sets instructor back to null so any other instructor can claim it.
