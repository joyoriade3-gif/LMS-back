...
// ── GET /api/v1/schedule/student/notifications ───────────────────────────────
// Live query, no separate notification collection: a "notification" is just
// any Schedule doc, created in the last 24 hours, for a course the student is
// actively enrolled in. It naturally stops appearing after 1 day — nothing
// to mark as read or expired.
export const getStudentScheduleNotifications = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const studentId = (req as any).user?.id;
    if (!studentId) return fail(res, 401, 'Unauthorized');
    ...
    const notifications = await Schedule.find({
      course: { $in: courseIds },
